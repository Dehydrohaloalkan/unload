...
- Очереди: `bigQueue` (скрипты target-кодов из `bigScripts`) и `lightQueue` (остальные скрипты); первые `N-1` worker-ов стартуют как `big-first`, остальные как `light-first`, при пустой своей очереди сразу забирают задачи из соседней.
...
- Во время выполнения показывает live-таблицу по количеству worker-потоков (`Runner.WorkerCount`) с текущим состоянием каждого потока (`running <script>` / `idle`) и последним событием раннера.
...
6. Big scripts (из `bigScripts`) приоритетно выполняются в n-1 потоках, остальные — в оставшихся потоках; worker в первую очередь берет "свои" скрипты (big-first/light-first), при пустой "своей" очереди сразу берет скрипты из другой. Для каждого скрипта:
