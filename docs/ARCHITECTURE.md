...
- `backend/Unload.FileWriter`
  ...
  - Запись чанков сериализована внутри writer (одна активная операция записи в момент времени), чтобы исключить межпоточное перемешивание строк при файловом выводе.
...
- `backend/Unload.Runner`
  ...
  - Чтение `DbDataReader` сериализовано (один активный reader в единицу времени) для совместимости с не-thread-safe реализациями `IDatabaseClient`/провайдеров БД; этапы записи файлов и публикации событий остаются параллельными.
