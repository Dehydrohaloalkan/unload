...
`OutputTextEncoding` является единственной точкой выбора кодировки пользовательских артефактов.
`PipeSeparatedFileChunkWriter`, `ExtraOutputWriter` и `RunReportCsvWriter` записывают Windows-1251
без BOM; `PipeDelimitedFormatter.EstimateLineBytes` считает размер чанка в этой же кодировке.
Внутренние JSON snapshots и API/logging contracts этой настройкой не затрагиваются.

`RunStatePersistence` держит единственную синхронную writer-границу вокруг захвата текущего
in-memory состояния и его записи. Если несколько потоков запрашивают сохранение одновременно,
каждый ожидающий writer снимает состояние только после получения блокировки, поэтому более старый
вызов не может позднее записать устаревший snapshot. JSON-контракт остаётся version `1` с массивом
`Runs`.

`JsonFileStore<T>` сериализует snapshot во временный `.tmp`, затем заменяет основной файл через
`File.Move(..., overwrite: true)`. Перед заменой существующего файла его предыдущая корректная
версия атомарно сохраняется рядом с суффиксом `.bak`; после первой записи backup совпадает с
основным snapshot. Поэтому восстановление может откатить не более одного успешного сохранения.

При повреждении основной JSON-файл перемещается в уникальный quarantine-файл с суффиксом
`.corrupt-<timestamp>-<id>`. Если `.bak` корректен, он восстанавливается как основной snapshot,
а persistence получает writable-статус `recovered`. Если backup отсутствует, повреждён или его
невозможно восстановить, store остаётся доступным для чтения, получает статус `corrupted` либо
`degraded` и не позволяет автоматически записать поверх проблемы пустой snapshot.

Ошибка записи логируется как `Error`, после чего исходное исключение повторно выбрасывается:
вызывающий код больше не может принять операцию за успешно persisted.

После первой ошибки записи хранилище переходит в degraded mode. Уже применённая mutation остаётся
доступной для чтения в памяти, потому что rollback мог бы потерять единственную актуальную версию
состояния. Следующие mutations блокируются до изменения памяти и получают
`PersistenceUnavailableException` с путём и исходной причиной. Чтение остаётся доступным; после
устранения причины требуется рестарт, чтобы заново загрузить persisted state и снять degraded mode.

`GET /api/system/health` объединяет состояние `runs.json` и `task-history.json`. Для writable
состояний `healthy` и `recovered` endpoint отвечает `200`; если хотя бы один store имеет
`degraded` или `corrupted`, ответ имеет статус `503`. Контракт показывает только status,
`isWritable` и время изменения состояния — внутренние пути, тип исключения и stack trace наружу
не возвращаются.
...
Воспроизводимая матрица отказов основной выгрузки и текущее поведение при сбоях БД, файловой
системы, шлюза и перезапуске API приведены в [отчёте по аварийным сценариям](CHAOS_TEST_REPORT.md).
...
и переводит фокус на явную кнопку закрытия. После закрытия соответствующий error store очищается;
одинаковая ещё не закрытая ошибка повторно не открывает второй overlay.

Пока синхронный preset находится в состоянии `running`, Angular блокирует действия запуска main и extra даже в admin mode. Это UI-защита от быстрых конфликтующих кликов; серверные правила `TaskWorkflow` остаются источником истины.
...
| `GET` | `/api/system/health` | Доступность записи run-state и task history; `200` или `503` |
...
Файл записывается в Windows-1251 без BOM. То же правило применяется к extra-файлам и
`run-report.csv`; размерные границы чанков рассчитываются по байтам Windows-1251.
...
- После ошибки записи in-memory состояние доступно только для чтения, а следующие mutations
  блокируются; восстановление writable-режима требует устранения причины и рестарта процесса.
- SignalR events транслируются всем клиентам; авторизация и изоляция клиентов в текущем контракте не описаны.
- `RunStateStore` всё ещё отвечает за recovery; ordered writer вынесен в `RunStatePersistence`.
