...
`File.Move(..., overwrite: true)`. Ошибка чтения по-прежнему даёт пустое/default состояние и
warning. Ошибка записи логируется как `Error`, после чего исходное исключение повторно
выбрасывается: вызывающий код больше не может принять операцию за успешно persisted.
...
- После ошибки записи in-memory состояние доступно только для чтения, а следующие mutations
  блокируются; отдельный health endpoint для degraded persistence пока не реализован.
