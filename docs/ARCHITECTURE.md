...
- Ошибка записи JSON логируется и выбрасывается вызывающему коду, но in-memory mutation к этому
  моменту могла уже примениться; стратегия degraded health или rollback ещё не определена.
- SignalR events транслируются всем клиентам; авторизация и изоляция клиентов в текущем контракте не описаны.
- `RunStateStore` всё ещё совмещает CAS-обновления и recovery; ordered writer вынесен в `RunStatePersistence`.
